variable "create_cluster" {
  description = "Flag to create a cluster"
  type        = bool
  default     = true
}
...
variable "private_subnet_ids" {
  description = "Define subnet where VMs should spin up"
  type        = string
  default     = ""
}
...
variable "vpc_id" {
  description = "Define VPC where VMs should spin up"
  type        = string
  default     = ""
}
...
variable "cluster-vms" {
  description = "Cluster VM configuration"
  type = list(object({
    count         = string      # number of instances
    os            = string      # operating system for vm
    instance_type = string      # type of instance (default/dedicated)
    ami_username  = string      # default user of ami machine
    key_name      = string      # key pair name, if already created
    role          = string      # role of the vm in cluster
    tags          = map(string) # tags specific to cluster vm
  }))
}

variable "swarm_security_group_rules" {
  description = "Define security group rules for Docker swarm"
  type = list(object({
    type                     = string       # type of security rule (ingress or egress)
    protocol                 = string       # type of protocol ̰
    description              = string       # Description about security group rule
    from_port                = number       # start range of the port
    to_port                  = number       # end range of the port ̰
    cidr_blocks              = list(string) # cidr blocks of the network
    source_security_group_id = list(string) # security groups
  }))
  default = [{
    type                     = ""
    protocol                 = ""
